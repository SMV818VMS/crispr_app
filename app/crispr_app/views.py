...
crispr_app = Blueprint('crispr_app', __name__,
                     template_folder='templates',
                     static_folder='static')

ROWS_PER_PAGE = 500
PATH_TO_TABLE = 'app/crispr_app/static/total_processed.txt'
...
@crispr_app.route('/sayhello/<name>')
def sayhello(name):
    return render_template("hello.html", name=name)

@crispr_app.route('/table/<column>/<value_min>-<value_max>')
def return_filtered_table(column, value_min, value_max):
    return 'Filtering by column {}: from {} to {}'.format(column,
            value_min, value_max)

@crispr_app.route('/table/')
def return_whole_table():
    return crispr_app.send_static_file('total_processed.json')
